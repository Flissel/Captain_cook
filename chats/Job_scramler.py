...
class JobIdeaMerger:
    ...
    def merge_single_job(self, job_description, ideas_with_scores, ideas):
        """
        Merges a single job description with its matching ideas, including additional details from the ideas list.
                Args:
            job_description (dict): Description of the job.
            ideas_with_scores (list): List of matching ideas with scores.
                Example: [{"idea": "idea1", "score": 95}, ...]
            ideas (list): List of all ideas with detailed information.
                Example: [{"Feature": "idea1", "Purpose": "...", ...}, ...]
                Returns:
            None
        """
        # Add each idea for the given job
        for i, idea_data in enumerate(ideas_with_scores, start=1):
            # Find the matching idea details from the ideas list
            matching_idea = next((item for item in ideas if item["Feature"] == idea_data["idea"]), {})
            
            # Create the merged row with idea details
            merged_row = {
                "ID": i,
            ...
            # Add the additional details from the matching idea
            merged_row.update(matching_idea)
            
            # Add the row to merged data
            self.merged_data.append(merged_row)
