...
def prepare_docker_context(output_path: Path) -> Path:
    ...
    # For a local Supabase dev stack, set SUPABASE_ANON_KEY to the public
    # supabase-demo anon key (printed by `supabase start`); no key is
    # hardcoded here so no secret-shaped literal lives in the repo.
    ...
    env_lines.append(f"SUPABASE_ANON_KEY={_default_anon_key}")
