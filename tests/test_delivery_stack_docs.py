...
def test_compose_owns_local_services_and_gates_n8n_behind_profile() -> None:
    compose = (ROOT / "docker-compose.yml").read_text(encoding="utf-8")

    assert "  mailpit:" in compose
    assert "  mariadb:" in compose
    assert "  n8n:" in compose
    assert 'profiles: ["owned-n8n"]' in compose
    assert "ledger_data:/var/lib/mysql" in compose


def test_env_example_defaults_to_external_n8n_and_documents_local_ports() -> None:
