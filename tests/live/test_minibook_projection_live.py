...
def test_real_minibook_projects_plan_assignment_and_readback() -> None:
    ...
    demo_key = os.environ.get("CAPTAIN_DEMO_MINIBOOK_API_KEY", "").strip()
    client = (
        MinibookClient(
            "http://127.0.0.1:3456", demo_key, timeout_seconds=5.0
        )
        if demo_key
        else MinibookClient.from_hermes_profile(
    ...
    project = projector.ensure_project(
        f"Captain Cook {run_id}", "Durable agent delivery team"
    )
