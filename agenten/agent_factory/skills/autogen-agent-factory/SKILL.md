...
6. Run the matching leased validation action with `python -m agenten.agent_factory.evaluation_cli`. Its JSON output is the only acceptable build, real-case, or quality block. Build preflight uses a fresh digest-verified temporary workspace. Real-case execution additionally requires the explicit `--live-team-execution` host composition: Captain materializes the sealed manifest and prompts into AutoGen, evaluates the private holdout through its typed evaluator port, and computes outcomes and usage itself. Candidate `entrypoint_command` metadata and candidate stdout are non-authoritative and are never executed or trusted in the paid path.
