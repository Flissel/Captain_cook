...
def _codex_prompt(
    ...
    return (
        ...
        "repository-wide test suite. Before packaging, validate "
        "generated-candidate/team_manifest.json against "
        "FactoryAutoGenTeamManifestV1. The executable team manifest must use "
        "content-addressed system_prompt_ref entries, never inline system_prompt, "
        "and must omit host_tools; keep host_tools only in factory-candidate.json. "
        "pytest.live.demo is deferred to Captain's sealed holdout gate; record it "
        "as deferred, never as passed. "
        ...
        "and small bounded slices such as `Select-Object -First`. Finish only "
        "after creating: "
