...
class PlanningPolicy:
    ...
    def validate_enrichment(self, enrichment: "BatchEnrichment") -> None:
        ...
        if duplicate_tags:
            raise PlanningPolicyError(f"duplicate capability tags: {duplicate_tags}")
        ...
        visible = {self.fingerprint_case(case) for case in enrichment.golden_cases}
        hidden = {self.fingerprint_case(case) for case in enrichment.holdout_cases}
        if visible & hidden:
    ...
    def validate_run_isolation(self, enrichments: Iterable["BatchEnrichment"]) -> None:
        visible: set[str] = set()
        hidden: set[str] = set()
        for enrichment in enrichments:
            visible.update(self.fingerprint_case(case) for case in enrichment.golden_cases)
            hidden.update(self.fingerprint_case(case) for case in enrichment.holdout_cases)
        if visible & hidden:
            raise PlanningPolicyError("holdout content overlaps build-visible golden content")
