...
- [ ] **Step 3: Fix embedded repos as submodules** — `git rm --cached hermes-agent Autogen_AgentFarm`; `git submodule add https://github.com/NousResearch/hermes-agent.git hermes-agent` (pin 77d5b2d); `git submodule add https://github.com/Flissel/Autogen_AgentFarm.git Autogen_AgentFarm` (pin 9ac6fe9). (§2, §23)
- [ ] **Step 4: Clone smoke test** — `git clone --recursive` into a temp dir; confirm `hermes-agent/` and `Autogen_AgentFarm/` are non-empty. The fix is not done until the clone proves it. (§2)
...
**Files:** Modify `Autogen_AgentFarm/minibook/config.yaml` (create), `gateway/mirror.py` (wire accounts)
